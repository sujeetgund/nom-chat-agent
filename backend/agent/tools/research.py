...
async def _web_search_impl(query: str, top_k: int = 5) -> str:
    ...
    plain-text summary of results. We run it in a thread pool to avoid
    blocking the event loop.
    """
    wrapper = DuckDuckGoSearchAPIWrapper()
    start = time.time()
    try:
        results_text = await asyncio.to_thread(wrapper.run, query)
...
async def _research_impl(topic: str, top_k: int = 5, use_web: bool = False) -> str:
    ...
    # Local KB retrieval
    hits, retrieval_time_ms = await search_knowledge_base(topic, top_k=top_k)
